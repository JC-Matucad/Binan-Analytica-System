                    ...
                    <div class="row">
                        <div class="c1 mb card-1">
                            <div class="height-100">
                                <div class="card">
                                    <div class="head">
                                        <div class="title"><h2>Growth Rate</h2></div>
                                        <div class="back"><a href="../user/society.php">Back</a></div>
                                    </div>
                                    <div class="graph">
                                    <canvas id="growthrate" ></canvas><!-- displaying the chart from the script below -->
                                    </div>
                                </div>
                            </div>
                        </div> 
...
<?php
        ...
        <script>
            ...
            try{
                ...
                $growthrate = $dbcon->prepare("SELECT `Year` , `Total` FROM `population_growth_rate` ORDER BY `Year` ASC ");
                $growthrate->execute();
                $year = [];
                $total = [];
                while ($row=$growthrate->fetch(PDO::FETCH_ASSOC)) {
                    extract($row);
                    $year[] = $Year;
                    $total[] = (int)$Total;
                }
            ...
            const ltx = document.getElementById('growthrate').getContext('2d');
            const growthrate = new Chart(ltx, {
                type: 'line',
                data: {
                    labels: 
