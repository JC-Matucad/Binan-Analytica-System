                    ...
                    <div class="row">
                        <div class="c1 mb card-1">
                                <div class="card height-100">
                                    <div class="head">
                                        <div class="title"><h2>Total Population</h2></div>
                                        <div class="back"><a href="../user/population.php">Back</a></div>
                                    </div>
                                    <div class="graph">
                                        <canvas id="population"></canvas><!-- displaying the chart from the script below -->
                                    </div>
                                </div>
                        </div> 
                    </div>
...
<script>
    ...
    ?>
            ...
            try{
                ...
                $population = $dbcon->prepare("SELECT `Year` , `Total` FROM `total_population`ORDER BY `Year` ASC");
                $population->execute();
                $year = [];
                $total = [];
                while ($row=$population->fetch(PDO::FETCH_ASSOC)) {
                    extract($row);
                    $year[] = $Year;
                    $total[] = (int)$Total;
                }
            ...
            const ltx = document.getElementById('population').getContext('2d');
            const population = new Chart(ltx, {
                type: 'line',
                data: {
                    labels: 
