                    ...
                    <div class="row">
                        <div class="c1 mb card-1">
                            <div class="height-100">
                                <div class="card">
                                    <div class="head">
                                        <div class="title"><h2>Birth Rate</h2></div>
                                        <div class="back"><a href="../user/society.php">Back</a></div>
                                    </div>
                                    <div class="graph">
                                    <canvas id="birthrate" ></canvas><!-- displaying the chart from the script below -->
                                    </div>
                                </div>
                            </div>
                        </div> 
...
<?php
        ...
        <script>
            ...
            try{
                ...
                $birthrate = $dbcon->prepare("SELECT `Year` , `Total` FROM `birth_rate`ORDER BY `Year` ASC");
                $birthrate->execute();
                $year = [];
                $total = [];
                while ($row=$birthrate->fetch(PDO::FETCH_ASSOC)) {
                    extract($row);
                    $year[] = $Year;
                    $total[] = (int)$Total;
                }
            ...
            const ltx = document.getElementById('birthrate').getContext('2d');
            const birthrate = new Chart(ltx, {
                type: 'line',
                data: {
                    labels: 
