                    ...
                    <div class="row">
                        <div class="c1 mb card-1">
                            <div class="height-100">
                                <div class="card">
                                    <div class="head">
                                        <div class="title"><h2>Total Household</h2></div>
                                        <div class="back"><a href="../user/population.php">Back</a></div>
                                    </div>
                                    <div class="graph">
                                    <canvas id="household" ></canvas><!-- displaying the chart from the script below -->
                                    </div>
                                </div>
                            </div>
                        </div> 
    ...
    ?>
        <?Php 
            try{
                ...
                $household = $dbcon->prepare("SELECT `Year` , `Total` FROM `total_household`ORDER BY `Year` ASC");
                $household->execute();
                $year = [];
                $total = [];
                while ($row=$household->fetch(PDO::FETCH_ASSOC)) {
                    extract($row);
                    $year[] = $Year;
                    $total[] = (int)$Total;
                }
            ...
            const ltx = document.getElementById('household').getContext('2d');
            const household = new Chart(ltx, {
                type: 'line',
                data: {
                    labels: 
