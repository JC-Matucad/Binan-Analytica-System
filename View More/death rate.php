                    ...
                    <div class="row">
                        <div class="c1 mb card-1">
                            <div class="height-100">
                                <div class="card">
                                    <div class="head">
                                        <div class="title"><h2>Death Rate</h2></div>
                                        <div class="back"><a href="../user/society.php">Back</a></div>
                                    </div>
                                    <div class="graph">
                                    <canvas id="deathrate" ></canvas><!-- displaying the chart from the script below -->
                                    </div>
                                </div>
                            </div>
                        </div> 
...
<?php
        ...
        <script>
            ...
            try{
                ...
                $deathrate = $dbcon->prepare("SELECT `Year` , `Total` FROM `death_rate` ORDER BY `Year` ASC");
                $deathrate->execute();
                $year = [];
                $total = [];
                while ($row=$deathrate->fetch(PDO::FETCH_ASSOC)) {
                    extract($row);
                    $year[] = $Year;
                    $total[] = (int)$Total;
                }
            ...
            const ltx = document.getElementById('deathrate').getContext('2d');
            const deathrate = new Chart(ltx, {
                type: 'line',
                data: {
                    labels: 
