 ...
 if($_FILES['product_file']['name'])
 {
  ...
  if(end($filename) == "csv")
  {
   ...
   while($data = fgetcsv($handle))
   {
    ...
    $sql="SELECT * FROM `marital_status_population` where `Year`='$Year' AND `Status`='$Status' limit 1";
      $result = mysqli_query($connect,$sql);
      if(mysqli_num_rows($result)==1){
        header("location: maritalstatus.php?fail=1");
      }else{
        $query = "
        INSERT INTO `marital_status_population` 
        SET 
        `id` = '$Id', 
        `Year` = '$Year', 
        `Status` = '$Status', 
       ...
       header("location: maritalstatus.php?updation=1");
     }
...
$query = "SELECT * FROM `marital_status_population`";
...
<html>
 ...
 <body class="modal-open">
   ...
   <div class="modal fade" id="deletemodal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel"
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <form action="action/delete_maritalstatus.php" method="POST">
  ...
  <div class="container">
   <h2>Import Marital Status Data</h2>
