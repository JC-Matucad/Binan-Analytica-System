 ...
 if($_FILES['product_file']['name'])
 {
  ...
  if(end($filename) == "csv")
  {
   ...
   while($data = fgetcsv($handle))
   {
    ...
    $sql="SELECT * FROM `total_household` where `Year`='$Year' limit 1";
      $result = mysqli_query($connect,$sql);
      if(mysqli_num_rows($result)==1){
        header("location: total_household.php?fail=1");
      }else{
        $query = "
        INSERT INTO `total_household` 
        SET
        `Year` = '$Year', 
        `Total` = '$Total'
        ";
        mysqli_query($connect, $query); 
        header("location: total_household.php?updation=1");
      }
...
$query = "SELECT * FROM `total_household`";
...
<html>
 ...
 <body class="modal-open">
   ...
   <div class="modal fade" id="deletemodal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel"
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <form action="action/delete_total_household.php" method="POST">
  ...
  <div class="container">
   <h2>Import Household Population Data</h2>
