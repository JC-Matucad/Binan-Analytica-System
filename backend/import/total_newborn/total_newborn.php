...
if(isset($_POST["upload"]))
{
 if($_FILES['product_file']['name'])
 {
    ...
    {
      $Year = mysqli_real_escape_string($connect, $data[0]);  
      $Total = mysqli_real_escape_string($connect, $data[1]);
      $sql="SELECT * FROM `total_newborn` where `Year`='$Year' limit 1";
        $result = mysqli_query($connect,$sql);
        if(mysqli_num_rows($result)==1){
          header("location: total_newborn.php?fail=1");
        }else{
          $query = "
          INSERT INTO `total_newborn` 
          SET 
          `Year` = '$Year', 
          `Total` = '$Total'
          ";
      mysqli_query($connect, $query); 
          header("location: total_newborn.php?updation=1");
        }
...
$query = "SELECT * FROM `total_newborn`";
...
<html>
 ...
 <body class="modal-open">
   ...
   <div class="modal fade" id="deletemodal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel"
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <form action="action/delete_total_newborn.php" method="POST">
   ...
   <div class="navbar"><h1>Biñan Analytica<h1>
     <a href="../../manage_data.php"><h1><i class="fa fa-solid fa-circle-arrow-left fa-lg"></i></h1></a>
   </div>
  <div class="container">
   <h2>Import Newborn Data</h2>
