 ...
 if($_FILES['product_file']['name'])
 {
  ...
  if(end($filename) == "csv")
  {
   ...
   while($data = fgetcsv($handle))
   {
    ...
    $sql="SELECT * FROM `poverty_rate` where `Year`='$Year' limit 1";
      $result = mysqli_query($connect,$sql);
      if(mysqli_num_rows($result)==1){
        header("location: poverty_rate.php?fail=1");
      }else{
        $query = "
        INSERT INTO `poverty_rate` 
        SET 
        `id` = '$Id', 
        `Year` = '$Year', 
        `Total` = '$Total', 
       ...
       header("location: poverty_rate.php?updation=1");
     }
...
$query = "SELECT * FROM `poverty_rate`";
...
<html>
 ...
 <body class="modal-open">
    ...
    <div class="modal fade" id="deletemodal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel"
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <form action="action/delete_poverty_rate.php" method="POST">
  ...
  <div class="container">
   <h2>Import Poverty Rate Data</h2>
