 ...
 if($_FILES['product_file']['name'])
 {
  ...
  if(end($filename) == "csv")
  {
   ...
   while($data = fgetcsv($handle))
   {
    ...
    $Cause = mysqli_real_escape_string($connect, $data[2]);
    $Total = mysqli_real_escape_string($connect, $data[3]);
    $sql="SELECT * FROM `cause_of_death` where `Year`='$Year' AND `Cause`='$Cause' limit 1";
      $result = mysqli_query($connect,$sql);
      if(mysqli_num_rows($result)==1){
        header("location: cause_of_death.php?fail=1");
      }else{
        $query = "
    ...
    mysqli_query($connect, $query); 
        header("location: cause_of_death.php?updation=1");
      }
...
$query = "SELECT * FROM `cause_of_death`";
...
<html>
 ...
 <body class="modal-open">
   ...
   <div class="modal fade" id="deletemodal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel"
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <form action="action/delete_cause_of_death.php" method="POST">
  ...
  <div class="container">
   <h2>Import Cause of Death  Population Data</h2>
   ...
   <?php echo $message; ?>
   <h3>Cause of Death Population</h3>
   <br />
