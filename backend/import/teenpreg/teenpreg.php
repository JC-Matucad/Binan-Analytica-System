 ...
 if($_FILES['product_file']['name'])
 {
  ...
  if(end($filename) == "csv")
  {
   ...
   while($data = fgetcsv($handle))
   {
    ...
    $sql="SELECT * FROM `teenage_pregnancy_population` where `Year`='$Year' limit 1";
      $result = mysqli_query($connect,$sql);
      if(mysqli_num_rows($result)==1){
        header("location: teenpreg.php?fail=1");
      }else{
        $query = "
        INSERT INTO `teenage_pregnancy_population` 
        SET 
        `id` = '$Id', 
        `Year` = '$Year', 
        `Total` = '$Total', 
       ...
       header("location: teenpreg.php?updation=1");
     }
...
$query = "SELECT * FROM `teenage_pregnancy_population`";
...
<html>
 ...
 <body class="modal-open">
   ...
   <div class="modal fade" id="deletemodal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel"
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <form action="action/delete_teenpreg.php" method="POST">
  ...
  <div class="container">
   <h2>Import Teenage pregnancy population Data</h2>
   ...
   <?php echo $message; ?>
   <h3>Teenage pregnancy population</h3>
   <br />
