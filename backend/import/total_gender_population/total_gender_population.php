 ...
 if($_FILES['product_file']['name'])
 {
  ...
  if(end($filename) == "csv")
  {
   ...
   while($data = fgetcsv($handle))
   {
      ...
      $Gender = mysqli_real_escape_string($connect, $data[2]);
      $Total = mysqli_real_escape_string($connect, $data[3]);
      $sql="SELECT * FROM `total_gender_population` where `Year`='$Year' AND `Gender`='$Gender' limit 1";
      $result = mysqli_query($connect,$sql);
        if(mysqli_num_rows($result)==1){
          header("location: total_gender_population.php?fail=1");
        }else{
            $query = "
            INSERT INTO `total_gender_population` 
            SET 
            `id` = '$Id', 
            `Year` = '$Year', 
            `Gender` = '$Gender', 
        ...
        header("location: total_gender_population.php?updation=1");
      }
...
$query = "SELECT * FROM `total_gender_population` ORDER BY `Year`";
...
<html>
 ...
 <body class="modal-open">
   ...
   <div class="modal fade" id="deletemodal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel"
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <form action="action/delete_total_gender_population.php" method="POST">
  ...
  <div class="container">
   <h2>Import Gender Population Data</h2>
