 ...
 if($_FILES['product_file']['name'])
 {
  ...
  if(end($filename) == "csv")
  {
   ...
   while($data = fgetcsv($handle))
   {
    ...
    $Barangay = mysqli_real_escape_string($connect, $data[2]);
    $Total = mysqli_real_escape_string($connect, $data[3]);
    $sql="SELECT * FROM `Barangay_population` where `Year`='$Year' AND `Barangay`='$Barangay' limit 1";
    $result = mysqli_query($connect,$sql);
    if(mysqli_num_rows($result)==1){
      header("location: Barangay_population.php?fail=1");
    }else{
      $query = "
   INSERT INTO `barangay_population` 
   SET 
   `id` = '$Id',
   `Year` = '$Year', 
   `Barangay` = '$Barangay', 
  ...
  mysqli_query($connect, $query); 
        header("location: barangay_population.php?updation=1");
      }
...
$query = "SELECT * FROM `barangay_population`";
...
<html>
 ...
 <body class="modal-open">
   ...
   <div class="modal fade" id="deletemodal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel"
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <form action="action/delete_barangay_population.php" method="POST">
  ...
  <div class="container">
   <h2>Import Barangay Population Data</h2>
